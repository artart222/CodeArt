...
if is_plugin_installed("lazy.nvim") and not disable_plugins.lazy then
    which_key.register({
        p = {
            name = "Plugin Manager",
            i = { ":Lazy install<CR>", "Install CodeArt packages" },
            u = { ":Lazy update<CR>", "Update CodeArt packages" },
            r = { ":Lazy clean<CR>", "Uninstall unnecessary CodeArt packages" },
            s = { ":Lazy sync<CR>", "Sync CodeArt packages" },
            p = { ":Lazy profile<CR>", "Show detailed profiling" },
            l = { ":Lazy log<CR>", "Show lazy.nvim logs" },
            d = { ":Lazy debug<CR>", "Show lazy.nvim debugger" },
            h = { ":Lazy home<CR>", "Show home page of lazy.nvim" },
            c = { ":Lazy check<CR>", "Check for updates and show the logs" },
        },
    }, { prefix = "<leader>" })
end
...
if is_plugin_installed("nvim-tree.lua") then
    ...
    which_key.register({
        n = {
            ...
            e = { nt_explorer, "Fullscreen Explorer" },
        },
    }, { prefix = "<leader>" })
end
